...
describe('Compra Multi Cartões com Cupom TROCA20', () => {

    it('Deve comprar mais de um item, usar cupom TROCA20 em um deles, ratear em 2 cartões e concluir fluxo no adminvendas', () => {
        const timestamp = Date.now();
        const nomeCliente = `Felipe Troca20 ${timestamp}`;
        const email = `felipe_troca_${timestamp}@teste.com`;
        ...
        // Insere o cupom de troca TROCA20 no primeiro item
        cy.get('#troca-0').type('TROCA20');

        // Faz o rateio do valor total
        cy.get('#total')
            .invoke('text')
            .then(texto => {
        ...
        // ============================
        // 4. ADMIN VENDAS - AVALIAÇÃO E CICLO DE VIDA DO PEDIDO
        // ============================
        cy.visit('http://localhost:8080/adminvendas.html');

        // Valida cupom no adminvendas
